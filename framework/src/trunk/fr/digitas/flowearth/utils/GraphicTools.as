...
package fr.digitas.flowearth.utils {
	...
	public class GraphicTools {
		...
		public static function fillRoundedCircleSegment( g : Graphics, radius1: Number, radius2: Number, startAngle: Number, endAngle: Number, round : Number = 0 , unit:String = "radians") : void
		{
			...
			g.moveTo( ax1a, ay1a );
			g.curveTo( ax1c, ay1c, ax1b, ay1b );
			
			g.lineTo( ax2a, ay2a );
			g.curveTo( ax2c, ay2c, ax2b, ay2b );
			...
			for( i = 0 ; i < segs2 ; i++ )
			{
				...
				bx = Math.cos( angle2 ) * radius2;
				by = Math.sin( angle2 ) * radius2;
				cx = Math.cos( angleMid ) * ( radius2 / cosTheta2 );
				cy = Math.sin( angleMid ) * ( radius2 / cosTheta2 );
			...
			g.curveTo( ax2c, ay2c, ax2a, ay2a );
			g.lineTo( ax1b, ay1b );
			g.curveTo( ax1c, ay1c, ax1a, ay1a );
			...
			for( i = segs1-1 ; i > -1 ; i-- )
			{
				...
				bx = Math.cos( angle1 ) * radius1;
				by = Math.sin( angle1 ) * radius1;
				cx = Math.cos( angleMid ) * ( radius1 / cosTheta1 );
				cy = Math.sin( angleMid ) * ( radius1 / cosTheta1 );
				g.curveTo( cx, cy, bx, by );
			}
		}
